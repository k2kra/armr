...
class MICRON(nn.Module):
    def __init__(self, vocab_size, ddi_adj, emb_dim=256, device=torch.device('cuda:0')):
        ...
        self.embeddings = nn.ModuleList(
            [nn.Embedding(vocab_size[i], emb_dim) for i in range(2)]).to(device)
    ...
    def forward(self, input):
        ...
        diag_emb = sum_embedding(
            self.dropout(
                self.embeddings[0](
                    torch.LongTensor(input[-1][0]).unsqueeze(dim=0).to(self.device)
                )
            )
        ) # (1,1,dim)
...
if __name__ == '__main__':
    from utils_ import get_data
    data,voc,ddi_A=get_data('../data/mimic-iii/', items=16)
    voc_size = (len(voc['diag_voc']['idx2word']), len(voc['pro_voc']['idx2word']), len(voc['med_voc']['idx2word']))
    model=MICRON(voc_size,ddi_A)
    # %%
    for p in data:
        model(p)
# %%
