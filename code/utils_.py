...
def get_tensor_data(dataset_path, visit_num, items=-1):
    data,voc,ddi_A = get_data(dataset_path, items)
...
def create_batches_dict(data, batch_size,voc_size, device):
    tensors=create_batches(*create_dataset(data,voc_size,device=device), batch_size=batch_size, sort=True)
    return {
